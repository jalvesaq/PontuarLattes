...
professores. Cada coordenador de curso deve consultar os documentos de
orientação da sua área e ficar atento neste relatório às tabelas mais
...
está o uso do JSR para a classificação da produção. Por isso, o relatório
inclui uma tabela em com a produção dos professores pontuada pelo índice SJR
dos periódicos em que os seus artigos foram publicados.
...
solicitamos que nos sejam comunicados qualquer erro encontrado nas tabelas e
gráficos.
...
MediaMovel <- function(x)
{
    if(nrow(x) < 3){
        ...
        if(nrow(x) == 0)
            cat("nenhum artigo foi publicado.\n\n")
        else if(nrow(x) == 1)
            cat("apenas 1 artigo foi publicado.\n\n")
        else
            cat("apenas 2 artigos foram publicados.\n\n")
        return(NA)
    }
