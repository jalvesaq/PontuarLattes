...
if(length(oaGini)){
    ...
    cat("Considerando apenas orientações de mestrado e doutorado, o índice de Gini do número de orientações em andamento por professor é de ",
        oaGini, ".\n\n", sep = "")
}
} else {
    Tabela(oriandTab, lbl = "tab:oriand", cap = "Orientações em andamento na data de atualização do currículo")
}
@
...
de classificação Qualis dos periódicos. Livros e capítulos de livros, aqui, estão,
arbitrariamente, recebendo pontuação equivalente a L2.
...
mediana de \textbf{\Sexpr{pmedianaA}}. No cálculo das médias ponderadas,
está sendo atribuído peso
...
A Tabela \ref{tab:idi} mostra o idioma detectado pela análise automática do
títulos dos artigos e capítulos ou de livro.\footnote{Função
...
idi1[til & (is.na(idi1) | idi1 == "GALICIAN")] <- "PORTUGUESE"
...
A Tabela \ref{tab:ttldif} tem o objetivo de ajudar a identificar erros de
...
Tabela~\ref{tab:proddet} quem foi o professor que publicou no periódico,
