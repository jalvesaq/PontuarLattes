...
<<ppg_pontos_lvr>>=
Tabela(pontuacaoLvr, cap = "Professores classificados por pontuação (Livros)",
...
<<ppg_pontos_art>>=
Tabela(pontuacaoArt, cap = "Professores classificados por pontuação (Artigos)",
       hline = nrow(pontuacaoLvr) - 1)
@
...
<<nSjr>>=
tab <- cbind("Professor" = rownames(nSJR), nSJR)
Tabela(tab, lbl = "tab:nsjr", hline = nrow(tab) - 1,
...
<<nSnip>>=
tab <- cbind("Professor" = rownames(nSnip), nSnip)
Tabela(tab, lbl = "tab:nSnip", hline = nrow(tab) -1,
...
<<ppg_pontos_snip>>=
Tabela(pontuacaoSNIP, lbl = "tab:snip", hline = nrow(tab) - 2,
