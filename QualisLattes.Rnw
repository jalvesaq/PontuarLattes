...
ferramenta contribuir para uma transparência e publicidade dos processos de
avaliação.
...
\section{Titulos e prêmios}

\begin{longtable}{lllcc}
\caption{Curso de doutorado} \\
  \toprule
<<tabelaGeral>>=
...
print(xtable(pontuacaoSNIP, digits = 3, label = "tab:snip",
      ...
      hline.after = c(-1, 0, nrow(pontuacaoSNIP) - 1, nrow(pontuacaoSNIP)))
@
