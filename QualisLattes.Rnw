...
\begin{longtable}{cclrl}
\caption{Links para ORCID e Lattes, atualização do Currículo Lattes e bolsa PQ} \\
  \toprule
    \multicolumn{2}{c}{\textbf{Links}} & \textbf{Professor} & \textbf{Dias} & \textbf{Bolsa PQ} \\
<<printDataLattes>>=
quando$nivelarea <- paste0(quando$nivel, " (", quando$area, ")")
quando$nivelarea[is.na(quando$nivel)] <- NA
dias <- cbind("ORCID" = paste0("\\href{", quando[, "orcid"], "}{\\includegraphics[width=4mm]{orcid.png}}"),
              ...
              "Dias" = quando[, "Dias"],
              "Bolsa PQ" = quando$nivelarea)
dias[quando[, "orcid"] == "", "ORCID"] <- ""
out <- capture.output(print(xtable(dias, align = "lcclrl", caption = "Links para ORCID e Lattes e data de atualização do Currículo Lattes"),
                     booktabs = TRUE, only.contents = TRUE, include.rownames = FALSE))
...
if(!is.null(doutor)){
    cat("\\begin{longtable}{L{7cm}L{7cm}L{7cm}cc}\n")
    ...
    cat("    \\multicolumn{4}{l}{Tempo mediano de titulação}  & ", round(median(as.numeric(tab[, 5]))), "\\\\\n")
...
print(xtable(posdoc, align = "lL{55mm}L{13cm}cc", label = "tab:posdoc",
...
if(length(premios) == 0){
    ...
    if(nrow(premios) == 0){
        ...
        print(xtable(premios, align = "llL{10cm}L{7.5cm}c",
...
print(xtable(oriconcTab, align = aln, digits = 0, label = "tab:oriconc",
      ...
      booktabs = TRUE, hline.after = c(-1, 0, nrow(oriconcTab) - 3, nrow(oriconcTab)),
      include.rownames = FALSE)
@
...
print(xtable(oc, align = "lcL{5.5cm}L{4cm}L{4cm}L{4cm}c", label = "tab:detoriconc",
...
print(xtable(oriandTab, align = aln, digits = 0, label = "tab:oriand",
      ...
      hline.after = c(-1, 0, nrow(oriandTab) - 3, nrow(oriandTab)), include.rownames = FALSE)
@

\newpage

<<oriandTab, warning=FALSE>>=
tab <- capture.output(print(xtable(oa, align = "lcL{4.5cm}cL{8cm}L{8cm}",
                                   label = "tab:oriandDet",
...
if(ncol(extensaoTab) == 1){
    ...
    aln <- paste0("llL{10cm}", paste0(rep("r", ncol(extensaoTab)-2), collapse = ""))
...
if(ncol(projextTab) == 1){
    print(xtable(projextTab, label = "tab:projext",
                 caption = paste0("Registro de ``Projetos de Extensão'' no período ",
                                  Inicio, "--", Fim)),
          include.rownames = FALSE)
} else {
    aln <- paste0("llL{10cm}", paste0(rep("r", ncol(projextTab)-2), collapse = ""))
...
print(xtable(pontuacaoLvr, digits = 0,
      ...
      hline.after = c(-1, 0, nrow(pontuacaoLvr)-2, nrow(pontuacaoLvr)))
@
...
print(xtable(pontuacaoArt, digits = 0,
      ...
      hline.after = c(-1, 0, nrow(pontuacaoArt)-2, nrow(pontuacaoArt)))
@
...
<<nSjr>>=
print(xtable(nSJR, digits = 0, label = "tab:nsjr",
             align = paste0("l", paste0(rep("r", ncol(nSJR)), collapse = "")),
             caption = "Percentual de artigos publicados em periódicos com SJR"),
      hline.after = c(-1, 0, nrow(nSJR) - 2, nrow(nSJR)))
@
...
print(xtable(pontuacaoSJR, digits = 3, label = "tab:sjr",
      ...
      hline.after = c(-1, 0, nrow(pontuacaoSJR) - 3, nrow(pontuacaoSJR)))
@
...
<<nSnip>>=
print(xtable(nSnip, digits = 0, label = "tab:nSnip",
             align = paste0("l", paste0(rep("r", ncol(nSnip)), collapse = "")),
             caption = "Percentual de artigos publicados em periódicos com SNIP"),
      hline.after = c(-1, 0, nrow(nSnip) - 2, nrow(nSnip)))
@
...
print(xtable(pontuacaoSNIP, digits = 3, label = "tab:snip",
      ...
      hline.after = c(-1, 0, nrow(pontuacaoSNIP) - 3, nrow(pontuacaoSNIP)))
@
...
\begin{tabular}{crl}
 ...
 ~~~~~~~~~ &  \textbf{V} & Coincidência de ISSN/ISBN, ano, tipo de produção
    (livro, artigo ou capítulo), volume, número e página inicial. \\
\end{tabular}

\begin{longtable}{|L{3cm}|L{6cm}|c|L{5cm}|r|r|r|r|}
\caption{Trabalhos produzidos em coautoria} \\
  \hline
    \multirow{2}{*}{\textbf{Professor}} & \multirow{2}{*}{\textbf{Produção}} &
    \multirow{2}{*}{\textbf{Ano}} & \multirow{2}{*}{\textbf{Livro ou
    periódico}} & \multirow{2}{*}{\textbf{ISSN ou ISBN}} & \multicolumn{3}{c|}{\textbf{N. Aut.}} \\
...
print(xtable(ttldif, align = "lL{13cm}L{10cm}",
