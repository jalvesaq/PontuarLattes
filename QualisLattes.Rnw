...
Seguindo os valores da Tabela \ref{tab:pontos} (nos períodos definidos na
Tabela~\ref{tab:qQmm}), a pontuação total média dos professores do
...
A Tabela \ref{tab:idi} mostra o idioma detectado pela análise automática dos
...
tab$Idioma <- sapply(sapply(strsplit(tab$Idioma, ""), function(x) c(x[1], sapply(x[2:length(x)], tolower))), paste0, collapse = "")
...
calculada como a soma do índice SJR \Sexpr{SJR_SNIP_version["SJR"]} de cada
publicação.
...
periódicos com SNIP (\Sexpr{SJR_SNIP_version["SNIP"]}) por professor e a
Tabela~\ref{tab:snip} mostra a pontuação calculada como a soma do índice SNIP
de cada publicação:
