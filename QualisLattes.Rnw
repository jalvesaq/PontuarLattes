...
falta <- !(doutorado[, "Professor"] %in% rownames(pontuacaoLvr))
if(sum(falta) > 0){
    falta <- doutorado[falta, "Professor"]
...
if(ncol(pontuacaoLvr) > 2){
    ...
    pmedia <- round(mean(pontuacaoLvr$Total, na.rm = TRUE))
    pmediana <- round(median(pontuacaoLvr$Total, na.rm = TRUE))
    pontuacaoLvr <- pontuacaoLvr[order(pontuacaoLvr$Total, decreasing = TRUE), ]
} else {
    pmedia <- round(mean(pontuacaoLvr[, 2], na.rm = TRUE))
    pmediana <- round(median(pontuacaoLvr[, 2], na.rm = TRUE))
...
# Adicionar professores que não produziram no período
falta <- !(doutorado[, "Professor"] %in% rownames(pontuacaoArt))
if(sum(falta) > 0){
    falta <- doutorado[falta, "Professor"]
...
if(ncol(pontuacaoArt) > 2){
    ...
    pmedia <- round(mean(pontuacaoArt$Total, na.rm = TRUE))
    pmediana <- round(median(pontuacaoArt$Total, na.rm = TRUE))
    pontuacaoArt <- pontuacaoArt[order(pontuacaoArt$Total, decreasing = TRUE), ]
} else {
    pmedia <- round(mean(pontuacaoArt[, 2], na.rm = TRUE))
    pmediana <- round(median(pontuacaoArt[, 2], na.rm = TRUE))
...
classificados pelo comitê de \emph{\Sexpr{NomeComite}} da CAPES, podem não ter sido
...
para a área de \emph{\Sexpr{NomeComite}}.
...
A área de \Sexpr{NomeComite} atribui peso \Sexpr{PesoLivros} aos livros e
\Sexpr{PesoArtigos} aos artigos. A Tabela~\ref{tab:pond} apresenta a
classificação dos professores quando sua produção é ponderada de acordo com os
critérios do comitê de área.
...
xtable(pond, digits = 3, label = "tab:pond",
